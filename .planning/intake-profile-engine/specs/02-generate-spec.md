...
- 2026-06-05: **gate-auto added for unattended runs (e2e + worker), Joshua-authorized.** The Continue gate (§4) had no non-interactive *first-approval* path: `--yes` honors only a previously-approved doc, and a non-TTY skips (leaves the tier unapproved). Correct for live clients, wrong for the autonomous e2e synthetic run and the future worker enqueue. Added env `UPRIVER_GATE_AUTO=1` → `resolveGateDecision` returns a new, distinct `'auto-approve-gate'` decision (highest precedence, over `--yes`/TTY/prior-approval) so a never-approved doc/tier auto-approves; both call sites (`generate/engine.ts` per-doc, `commands/generate.ts` per-tier) log loudly `[gate] AUTO-APPROVED (UPRIVER_GATE_AUTO) — unattended/synthetic runs only.`. The param is optional and defaults off, so with the env unset behavior is byte-identical and every prior gate test stays green (full `@upriver/cli` suite 369/369, +3 new gate tests). Intended only for unattended/synthetic runs; live client gates remain Joshua's interactive act.
