...
export interface PreflightResult {
  ok: boolean;
  available: { lighthouse: boolean; squirrelscan: boolean; playwrightBrowsers: boolean };
...
export function runPreflight(): PreflightResult {
  ...
  const playwrightBrowsers = hasPlaywrightBrowsers();
  ...
  if (!anthropicKey) warnings.push('ANTHROPIC_API_KEY not set — design-deep, accessibility-deep, CWV-deep, trust-signals passes will be skipped.');
  if (!playwrightBrowsers) warnings.push('Playwright browsers not installed — run `npx playwright install`. Analytics-tracking and cross-browser passes will be skipped.');
  ...
  return {
    ...
    available: { lighthouse, squirrelscan, playwrightBrowsers },
    skills,
    anthropicKey,
    warnings,
  };
...
function hasPlaywrightBrowsers(): boolean {
  try {
    const out = execSync('npx playwright --version', { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
