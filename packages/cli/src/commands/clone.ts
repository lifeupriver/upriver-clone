...
- **Forms:** if the source has any form (newsletter signup, contact / inquiry, RSVP, etc.), recreate it in Astro — do NOT just embed an external link or skip it. Extract every \`<input>\`, \`<select>\`, \`<textarea>\`, and \`<button type="submit">\` from rawhtml, preserving \`name\`, \`type\`, \`required\`, \`placeholder\`, and any visible labels. Wire the form's \`action\` to a same-origin endpoint:
  ...
  Set \`method="POST"\` and \`enctype="application/x-www-form-urlencoded"\` (or \`multipart/form-data\` if there's a file input). Reuse \`src/components/astro/ContactForm.astro\` if its structure matches the source — otherwise inline. Add a hidden honeypot field (\`<input type="text" name="_hp" class="hidden" tabindex="-1" autocomplete="off">\`) to deter bots. On success the endpoints answer plain form POSTs with a 303 redirect back to the referring page with \`?submitted=1\` appended (a hidden \`_redirect\` input with a root-relative path overrides the destination); include a confirmation region (\`<div data-form-status>\`) revealed by a tiny script when \`location.search\` contains \`submitted=1\` — see ContactForm.astro for the pattern. Do NOT auto-create the API route handlers — the scaffold provides \`src/pages/api/inquiry.ts\` and \`src/pages/api/newsletter.ts\`; just wire the form's \`action\` to them.
